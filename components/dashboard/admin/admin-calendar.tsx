...
const AdminCalendar = ({ timeOffRequests, companyHolidays, employees }: AdminCalendarProps) => {
  ...
  // Filter time off requests based on selected filters
  const filteredTimeOffRequests = useMemo(() => {
    if (statusFilter === "ALL") return timeOffRequests;
    return timeOffRequests.filter(request => request.status === statusFilter);
          ...
          <div className="space-y-0.5 max-h-12 overflow-hidden">
            ...
            {timeOffRequestsForDate.slice(0, 2).map(request => {
              ...
              return (
                <div 
                  key={request.id} 
                  className="text-[10px] p-0.5 rounded border truncate flex items-center gap-0.5"
                  style={{
                    backgroundColor: request.status === 'APPROVED' ? '#dcfce7' : 
                                   request.status === 'PENDING' ? '#fef9c3' : '#fee2e2',
                    borderColor: request.status === 'APPROVED' ? '#bbf7d0' : 
                                request.status === 'PENDING' ? '#fde047' : '#fca5a5',
                    color: request.status === 'APPROVED' ? '#166534' : 
                          request.status === 'PENDING' ? '#854d0e' : '#991b1b'
                  }}
                >
                  <StatusIcon className="w-2.5 h-2.5 flex-shrink-0" />
                  <span className="truncate">{request.employee.firstName}</span>
                </div>
              );
            })}
          ...
          <div className="space-y-1 max-h-20 overflow-y-auto">
            ...
            {timeOffRequestsForDate.map(request => {
              ...
              return (
                <div 
                  key={request.id} 
                  className="text-xs p-1 rounded border flex items-center gap-1"
                  style={{
                    backgroundColor: request.status === 'APPROVED' ? '#dcfce7' : 
                                   request.status === 'PENDING' ? '#fef9c3' : '#fee2e2',
                  }}
  ...
  const stats = useMemo(() => {
    ...
    const pendingRequests = filteredTimeOffRequests.filter(req => req.status === 'PENDING').length;

    // Calculate total leave days used (only approved requests)
    const totalLeaveDaysUsed = filteredTimeOffRequests
      .filter(req => req.status === 'APPROVED')
      .reduce((sum, req) => sum + (req.workingDaysCount || 0), 0);
  ...
  }, [employees, filteredTimeOffRequests]);
  ...
  return (
    <div className="min-h-screen p-3 space-y-3 bg-gray-50">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Company Calendar</h1>
          <p className="text-xs text-gray-600 mt-0.5">Manage and track employee time off</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="bg-white rounded-md border border-blue-200 p-1">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              ...
              <select 
                ...
                className="text-xs border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-transparent"
              >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        <div className="bg-white rounded-lg border border-blue-200 hover:shadow-sm transition-all duration-200 p-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-lg font-bold text-gray-900">{stats.totalEmployees}</h3>
            </div>
        ...
        <div className="bg-white rounded-lg border border-purple-200 hover:shadow-sm transition-all duration-200 p-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-gray-500">Leave Days Used(Total)</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-3">
        ...
        <div className="space-y-3">
          ...
          {selectedDate && (
            <div className="bg-white rounded-lg border border-blue-200">
              ...
              <div className="p-2 space-y-3">
                ...
                {/* Employee Leave Summary for Selected Date */}
                <div>
                  <h3 className="font-medium text-sm mb-2 flex items-center gap-1">
                  ...
                  <div className="space-y-3 max-h-60 overflow-y-auto">
                    {employees
                      .filter(employee => 
                        selectedDateRequests.some(request => request.employeeId === employee.id)
                      )
                      .map(employee => (
                        <div key={employee.id} className="p-3 border rounded-md bg-white">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                            ...
                            <Badge 
                              variant={employee.remainingLeave >= 0 ? "outline" : "destructive"} 
                              className="text-xs"
                            >
                          ...
                          <div className="grid grid-cols-2 gap-3 text-sm mb-2">
                            ...
                            <div>
                              <p className="text-gray-500 text-xs">Leave Taken</p>
                    ...
                    {selectedDateRequests.length === 0 && (
                      <div className="text-center py-4 border border-dashed border-gray-300 rounded">
                        ...
                        <p className="text-gray-500 text-xs">No time off requests for this date</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {selectedDateHoliday && (
                  <div className="p-2 bg-red-50 border border-red-200 rounded">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <h3 className="font-semibold text-red-800 text-xs">Holiday</h3>
                    </div>
          ...
          {/* Team Overview - Always shows */}
          <div className="bg-white rounded-lg border border-blue-200">
            <div className="p-2 border-b rounded-lg border-blue-200 bg-white">
              <div className="text-sm font-semibold flex items-center">
                <Users className="mr-1 h-3 w-3 text-green-600" />
                Team Overview
              </div>
            </div>
            <div className="p-2">
